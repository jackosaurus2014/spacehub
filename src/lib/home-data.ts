...
export interface UpcomingRow { id: string; name: string; launchDate: Date | null; rocket: string | null; location: string | null; agency: string | null; status: string }
export interface Story { href: string; title: string; summary: string; category: string; date: Date; type: 'ai-insight' | 'blog'; source: string }
...
async function loadHomeData(): Promise<HomeData> {
  ...
  const [nextSlips, upcoming, slipSeries, funding12mo, kp, stories, topCorps] = await Promise.all([
    ...
    safe('upcoming', () => getNextLaunches(6, now), [] as UpcomingRow[]),
    ...
    safe('stories', async () => {
      ...
      for (const i of insights) cards.push({ href: `/ai-insights/${i.slug}`, title: i.title, summary: i.summary, category: i.category, date: i.generatedAt, type: 'ai-insight', source: 'SpaceNexus analysis · fact-checked' });
      for (const p of BLOG_POSTS.slice(0, 4)) cards.push({ href: `/blog/${p.slug}`, title: p.title, summary: p.excerpt, category: p.category, date: new Date(p.publishedAt), type: 'blog', source: `${p.author} · ${p.readingTime} min` });
