...
export const RESEARCH_RELEASES: readonly ResearchRelease[] = [
  {
    ...
    methodology: [
      ...
      'Only PRIVATE SPACE ROUNDS are counted. A transaction must be primary private capital into the company — IPOs, direct listings, SPAC mergers, PIPEs and registered directs are public-market offerings; tender offers and secondaries move shares that already exist; debt is borrowing and grants are not investment. It must also go to a company our records classify as doing space work, which for cross-domain sectors such as defence and aerospace requires the recorded subsector to name space. Every excluded transaction is published in the edition with the reason. The full rule travels in each edition’s coverage block.',
      'A round is attributed to the quarter its announced date falls in, in UTC.',
      'A round counts toward an investor only when that investor is recorded as the LEAD. Participation without a lead credit is ranked in a separate table, because we do not hold complete participant lists and a partial one would rank the wrong firms.',
      'Jointly-led rounds are split. Our sources record co-leads as one string ("Eclipse / Riot Ventures"); we split on a slash with whitespace on both sides and on nothing else, because an ampersand or a comma can sit inside a real firm name, and credit each co-lead with one round led. Each edition publishes every string it split or renamed.',
      ...
      '"New to the table" means the investor led no counted round in the previous four quarters. It is a statement about our records, not about the firm.',
    ],
