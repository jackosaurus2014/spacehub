...
describe('rankShares', () => {
  it('computes sharePct against the supplied market total and sorts desc by value', () => {
    const fills: RawFill[] = [
  ...
  it('tags the NPC market maker distinctly and never excludes it from totals', () => {
    ...
    expect(npcRow.sharePct).toBe(100); // NPC is a real participant, not excluded
  });
