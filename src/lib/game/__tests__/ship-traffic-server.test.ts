...
describe('contactsFromProfiles', () => {
  it('derives transit progress and ETA from the synced route timestamps', () => {
    const pool = contactsFromProfiles([profile('p1', 'Acme', [inTransit('leo', 'lunar_orbit', NOW - 30_000, NOW + 90_000)])], NOW, '2026-09-13', 's');
  ...
  it('expresses progress from laneA to laneB even when the ship flies the lane backwards', () => {
    ...
    const pool = contactsFromProfiles([profile('p1', 'Acme', [inTransit('lunar_orbit', 'leo', NOW - 30_000, NOW + 90_000)])], NOW);
    expect(pool[0].pub.laneA).toBe('leo');
    expect(pool[0].pub.progress).toBeCloseTo(0.75, 6);
    expect(pool[0].destinationId).toBe('leo');
  });
  it('a ship whose arrival passed without a re-sync holds at its destination', () => {
    const pool = contactsFromProfiles([profile('p1', 'Acme', [inTransit('leo', 'ceres_surface', NOW - 200_000, NOW - 1)])], NOW);
    expect(pool[0].pub.status).toBe('holding');
    expect(pool[0].pub.locationId).toBe('ceres_surface');
  });
  it('idle/mining ships hold at their location; unbuilt, building and expedition ships are not in the pool', () => {
...
describe('buildFeedForRequester', () => {
  ...
  it('caps the feed and reports the true total', () => {
    ...
    const big = contactsFromProfiles([profile('p1', 'Acme', ships)], NOW);
