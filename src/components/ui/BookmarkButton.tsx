...
export default function BookmarkButton({ itemId, itemTitle, itemUrl, className = '' }: {
  ...
  return (
    <button
      onClick={toggle}
      className={`relative min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg transition-all ${saved ? 'text-white/70' : 'text-slate-400 hover:text-white/70'} ${className}`}
    ...
    >
      ...
      {flash && (
        <span className="absolute -top-8 left-1/2 -translate-x-1/2 text-xs text-white/70 bg-white/[0.06] px-2 py-1 rounded shadow-lg whitespace-nowrap animate-fade-in-up">
          Saved!
        </span>
      )}
    </button>
