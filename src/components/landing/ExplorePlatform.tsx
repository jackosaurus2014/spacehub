          ...
          {DESTINATIONS.map((dest) => (
            <motion.div key={dest.href} variants={cardVariants}>
              ...
              >
                {/* Status badge */}
                {dest.badge && (
                  <span className="absolute top-2.5 right-2.5 pro-badge">{dest.badge}</span>
                )}
                {!dest.badge && dest.stat === 'LIVE' && (
                  <span className="absolute top-2.5 right-2.5 live-badge text-[7px]">LIVE</span>
                )}
                {!dest.badge && dest.stat === 'NEW' && (
                  <span className="absolute top-2.5 right-2.5 text-[7px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-blue-500/15 text-blue-400 border border-blue-500/20">NEW</span>
                )}
                ...
                <p className="text-[11px] md:text-xs text-slate-500 group-hover:text-slate-400 transition-colors leading-relaxed">
                  {dest.description}
                </p>

                {/* Explore arrow */}
                <div className="flex items-center gap-1 text-[10px] text-slate-600 group-hover:text-slate-400 transition-colors mt-auto pt-1">
                  Explore
