...
/* ------------------------------------------------------------------ */
/*  Explore the Platform — 8 key destination cards in a 4x2 grid      */
/* ------------------------------------------------------------------ */

const DESTINATIONS = [
  {
    title: '100+ Blog Articles',
      ...
      <div className="container mx-auto px-4">
        <div className="mb-8 text-center">
          ...
          <p className="text-slate-400 text-lg max-w-2xl mx-auto">
            Eight ways to dive into the space industry&mdash;all free, all original.
          </p>
        </div>
        ...
        >
          {DESTINATIONS.map((dest) => (
            <motion.div key={dest.href} variants={cardVariants}>
              <Link
                href={dest.href}
                className="group flex flex-col gap-2 p-4 md:p-5 rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.05] hover:border-white/[0.12] transition-all duration-200 ease-smooth h-full"
              >
                <div className="flex items-center gap-2.5">
                  <span className="text-slate-400 group-hover:text-white transition-colors">
                    {dest.icon}
                  </span>
                  <h3 className="text-sm md:text-base font-semibold text-white/90 group-hover:text-white transition-colors leading-tight">
                    {dest.title}
                  </h3>
                </div>
