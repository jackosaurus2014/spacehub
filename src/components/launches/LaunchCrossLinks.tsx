...
export default function LaunchCrossLinks({ rocket, location, eventId, upcoming, hide = [], debriefSlug = null, alertsAnchor = false }: { alertsAnchor?: boolean; debriefSlug?: string | null; rocket?: string | null; location?: string | null; eventId?: string; upcoming?: boolean; hide?: string[] }) {
  ...
  // Tier 2 #12 (2026-09-06): the missing step in the rail. When the host page
  // carries a LaunchWatchForm it anchors to it; otherwise the alerts hub.
  items.push({ key: 'alerts', href: alertsAnchor ? '#launch-alerts' : '/alerts', icon: '✉️', label: 'Get launch alerts', hint: 'A day before, an hour before, and when it flies' });
  ...
  // Eight, not six: the alerts step would otherwise push the debrief off a
  // launch page. Four columns on desktop keeps it two even rows.
  const shown = items.filter((i) => !hide.includes(i.key)).slice(0, 8);
  if (shown.length === 0) return null;
  return (
    <nav aria-label="Related launch pages" className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
      {shown.map((i) => (
