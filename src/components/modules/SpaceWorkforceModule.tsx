...
function JobPostingCard({ job }: { job: SpaceJobPosting }) {
  ...
  return (
    <div className="bg-white/[0.04] rounded-lg p-4 hover:bg-white/[0.08] transition-all">
      <div className="flex items-start justify-between mb-2">
        <div className="flex-1 min-w-0">
          <h4 className="font-semibold text-white/90 text-sm truncate">{job.title}</h4>
          <p className="text-slate-400 text-xs">{job.company}</p>
        </div>
        {job.remoteOk && (
          <span className="text-xs bg-white/10 text-white/90 px-2 py-0.5 rounded ml-2 whitespace-nowrap">
            Remote
          </span>
        )}
      </div>
    ...
    <div className="bg-white/[0.04] rounded-lg p-4">
      ...
      <div className="grid grid-cols-2 gap-3 text-xs">
        <div>
          ...
          <div className="text-white/90 font-medium text-sm">{trend.totalOpenings.toLocaleString()}</div>
        </div>
        <div>
          <span className="text-slate-400">Avg Salary</span>
          <div className="text-white/90 font-medium text-sm">{formatSalary(trend.avgSalary ?? 0)}</div>
        </div>
        <div>
          <span className="text-slate-400">Hires</span>
          <div className="text-white/90 font-medium text-sm">{(trend.totalHires ?? 0).toLocaleString()}</div>
        </div>
        <div>
          <span className="text-slate-400">Median Salary</span>
          <div className="text-white/90 font-medium text-sm">{formatSalary(trend.medianSalary ?? 0)}</div>
        </div>
      </div>
  ...
  return (
    <div className="bg-white/[0.04] rounded-lg p-3">
      ...
      <div className="flex justify-between text-xs text-slate-400">
        ...
        <span className="text-white/90 font-medium">Median: {formatSalary(benchmark.avgMedian)}</span>
  ...
  if (!stats || (jobs.length === 0 && trends.length === 0)) {
    return (
      <div className="card p-8 text-center">
        ...
        <h3 className="text-xl font-semibold text-white/90 mb-2">Space Workforce & Talent Analytics</h3>
