...
/**
 * The interactive half of /space-talent — the former page.tsx, moved wholesale.
 *
 * The <h1>, hero, metadata and JSON-LD schemas now live in the SERVER
 * component (./page.tsx), which also reads the landing tab's first screen of
 * data at request time and hands it down as `initialTalent` /
 * `initialWorkforce`. Seeded state is NOT refetched on hydration (see the
 * skipFirst*Fetch refs) — pattern: company-profiles/CompanyProfilesClient.tsx
 * and news/NewsPageClient.tsx.
 */
...
function SpaceTalentHubContent({ initialTalent, initialWorkforce }: SpaceTalentClientProps) {
  ...
  // Talent state.
  // seededTalent === the server (page.tsx) rendered the default experts view;
  // the first effect pass must NOT refetch it (skipFirstTalentFetch below).
  const seededTalent = Boolean(initialTalent && initialTalent.talent.length > 0);
  const [talent, setTalent] = useState<SpaceTalent[]>(seededTalent ? initialTalent!.talent : []);
  const [talentStats, setTalentStats] = useState<TalentStats | null>(initialTalent?.stats ?? null);
  ...
  const [talentDataAsOf, setTalentDataAsOf] = useState<string | null>(() =>
    seededTalent ? formatAsOfDate(initialTalent!.asOf) : null
  );
  const skipFirstTalentFetch = useRef(seededTalent);
  ...
  // Jobs state.
  // seededJobs === the server (page.tsx) rendered the default, unfiltered
  // /api/workforce first screen; the first effect pass must NOT refetch it.
  const seededJobs = Boolean(initialWorkforce && initialWorkforce.jobs.length > 0);
  const [jobs, setJobs] = useState<SpaceJobPosting[]>(seededJobs ? initialWorkforce!.jobs : []);
  const [totalJobs, setTotalJobs] = useState(seededJobs ? initialWorkforce!.totalJobs : 0);
  const [jobsLoading, setJobsLoading] = useState(!seededJobs);
  const skipFirstJobsFetch = useRef(seededJobs);
  ...
  // Trends state (seeded alongside jobs — /api/workforce returns them together)
  const [trends, setTrends] = useState<WorkforceTrend[]>(initialWorkforce?.trends ?? []);
  const [trendsLoading, setTrendsLoading] = useState(!initialWorkforce);

  // Salary benchmarks state
  const [benchmarksByCategory, setBenchmarksByCategory] = useState<SalaryBenchmarkEntry[]>(
    initialWorkforce?.benchmarks.byCategory ?? []
  );
  const [benchmarksBySeniority, setBenchmarksBySeniority] = useState<SalaryBenchmarkEntry[]>(
    initialWorkforce?.benchmarks.bySeniority ?? []
  );
  const [salaryView, setSalaryView] = useState<'category' | 'seniority'>(initialSalaryView);
  const [salariesLoading, setSalariesLoading] = useState(!initialWorkforce);

  // Stats
  const [wfStats, setWfStats] = useState<WorkforceStats | null>(initialWorkforce?.stats ?? null);
  ...
  useEffect(() => {
    if (topTab === 'talent' && talentSubTab === 'experts') {
      // The server already rendered this exact default view — do not repeat
      // it on hydration. Every later run (a filter, a tab return) fetches.
      if (skipFirstTalentFetch.current) {
        skipFirstTalentFetch.current = false;
        return;
      }
      fetchTalent();
    }
  ...
  useEffect(() => {
    if (topTab === 'workforce') {
      // The server already rendered the default first screen — skip the
      // hydration fetch; every later run (filter, search, tab return) fetches.
      if (skipFirstJobsFetch.current) {
        skipFirstJobsFetch.current = false;
        return;
      }
      setJobsOffset(0);
      fetchJobs(0, false);
      ...
      {topTab === 'gigs' && (
        <div className="space-y-6">
          <div className="rounded-xl border border-white/20 bg-white/5 p-5">
            <h3 className="text-lg font-semibold text-white">Freelance &amp; contract talent</h3>
            <p className="text-sm text-slate-400 mt-1 max-w-2xl">
              Independent engineers, analysts, and specialists available for project work.
            </p>
          </div>
          <GigBoard />
        </div>
...
export default function SpaceTalentClient({ initialTalent, initialWorkforce }: SpaceTalentClientProps) {
  // The outer shell — <h1>, hero art, breadcrumb/item-list JSON-LD, metadata —
  // is rendered by the SERVER component (./page.tsx) so it exists in HTML for
  // crawlers. This island owns the tabs, filters, forms and fetches.
  return (
    <>
      <Suspense
        fallback={
      ...
      >
        <SpaceTalentHubContent initialTalent={initialTalent} initialWorkforce={initialWorkforce} />
      </Suspense>
