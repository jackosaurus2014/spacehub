...
import { BLOG_CATEGORIES, type BlogCategory, type BlogPostMeta } from '@/lib/blog-metadata';
...
interface BlogListingClientProps {
  /** All posts (metadata only), sorted newest first by the server component. */
  posts: BlogPostMeta[];
}
...
function BlogListingContent({ posts }: BlogListingClientProps) {
  ...
  const latestPostDate = useMemo(() => {
    ...
    return posts.reduce((latest, p) =>
      new Date(p.publishedAt).getTime() > new Date(latest.publishedAt).getTime() ? p : latest
    ).publishedAt;
  }, [posts]);
  ...
  return (
    <div className="min-h-screen pb-12">
      <div className="container mx-auto px-4">
        ...
        {/* 160+ Articles Social Proof Badge */}
        <div className="flex items-center justify-center mb-6">
          <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-gradient-to-r from-violet-500/[0.12] to-blue-500/[0.08] border border-violet-500/25">
            ...
            <span className="text-sm font-semibold text-white">200+ Original Articles</span>
        ...
        {latestPostDate && (
          <p className="text-xs text-slate-500 mb-6">
            Latest post: {formatDate(latestPostDate)}
          </p>
        )}
