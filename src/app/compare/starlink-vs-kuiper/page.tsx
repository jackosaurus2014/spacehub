...
export default async function Page() {
  const figures = await getCompareFigures(['spacex']);
  ...
  const comparisonData = COMPARISON_DATA.map((row) => {
    if (row.metric === 'Parent Company Market Cap') {
      return {
        ...row,
        a: spacexMarketCap ? `SpaceX public (~${spacexMarketCap}, SPCX, Aug 2026)` : row.a,
      };
    }
    return row;
  });
    ...
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      ...
      <div className="card-terminal mb-8">
        ...
        <div className="px-2 sm:px-4 pb-3">
          <CompareFiguresFootnote figures={[spacex]} />
        </div>
      </div>
