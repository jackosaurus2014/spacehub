...
export default async function Page() {
  const figures = await getCompareFigures(['pulsar-fusion']);
  const pulsarFusion = figures['pulsar-fusion'];
  const pulsarFunding = formatFundingTotal(pulsarFusion?.totalFundingUSD);

  const comparisonData = COMPARISON_DATA.map((row) => {
    if (row.metric === 'Total Funding') {
      return {
        ...row,
        a: pulsarFunding ? `~${pulsarFunding} (private, grants)` : row.a,
      };
    }
    return row;
  });
    ...
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      ...
      <div className="card-terminal mb-8">
        ...
        <div className="px-2 sm:px-4 pb-3">
          <CompareFiguresFootnote figures={[pulsarFusion, undefined]} />
        </div>
      </div>
