...
const COMPARISON_DATA = [
  ...
  { metric: 'Public / Private', a: 'Public (NYSE: SPIR, via SPAC 2021)', b: 'Public (Nasdaq: HAWK, IPO May 2026)' },
  { metric: 'Employees', a: '~750', b: '~300 (est.)' },
  { metric: 'Total Funding / Market Cap', a: '~$276M raised; market cap ~$200M (volatile)', b: '~$585M raised pre-IPO; public since May 2026 (Nasdaq: HAWK)' },
...
export default async function Page() {
  const figures = await getCompareFigures(['spire-global', 'hawkeye-360']);
  const spire = figures['spire-global'];
  const hawkeye = figures['hawkeye-360'];
  const spireFunding = formatFundingTotal(spire?.totalFundingUSD);
  const spireMarketCap = formatMarketCap(spire?.marketCapUSD);
  const hawkeyeFunding = formatFundingTotal(hawkeye?.totalFundingUSD);
  const hawkeyeMarketCap = formatMarketCap(hawkeye?.marketCapUSD);

  const comparisonData = COMPARISON_DATA.map((row) => {
    if (row.metric === 'Total Funding / Market Cap') {
      return {
        ...row,
        a: spireFunding && spireMarketCap ? `~${spireFunding} raised; market cap ~${spireMarketCap} (volatile)` : row.a,
        b: hawkeyeFunding ? `~${hawkeyeFunding} raised pre-IPO; public since May 2026 (Nasdaq: HAWK)${hawkeyeMarketCap ? `; market cap ~${hawkeyeMarketCap}` : ''}` : row.b,
      };
    }
    return row;
  });
    ...
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      ...
      <div className="card-terminal mb-8">
        ...
        <div className="px-2 sm:px-4 pb-3">
          <CompareFiguresFootnote figures={[spire, hawkeye]} />
        </div>
      </div>
