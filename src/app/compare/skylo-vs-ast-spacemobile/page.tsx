...
export default async function Page() {
  const figures = await getCompareFigures(['skylo', 'ast-spacemobile']);
  const skylo = figures['skylo'];
  const ast = figures['ast-spacemobile'];
  const skyloFunding = formatFundingTotal(skylo?.totalFundingUSD);
  ...
  const comparisonData = COMPARISON_DATA.map((row) => {
    if (row.metric === 'Total Funding') {
      return {
        ...row,
        a: skyloFunding ? `~${skyloFunding}` : row.a,
        b: astFunding ? `~${astFunding} (public: ASTS on NASDAQ)` : row.b,
      };
    }
    return row;
  });
    ...
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      ...
      <div className="card-terminal mb-8">
        ...
        <div className="px-2 sm:px-4 pb-3">
          <CompareFiguresFootnote figures={[skylo, ast]} />
        </div>
      </div>
