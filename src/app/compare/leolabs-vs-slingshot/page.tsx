...
export default async function Page() {
  const figures = await getCompareFigures(['leolabs', 'slingshot-aerospace']);
  const leolabs = figures['leolabs'];
  const slingshot = figures['slingshot-aerospace'];
  const leolabsFunding = formatFundingTotal(leolabs?.totalFundingUSD);
  const slingshotFunding = formatFundingTotal(slingshot?.totalFundingUSD);

  const comparisonData = COMPARISON_DATA.map((row) => {
    if (row.metric === 'Total Funding') {
      return {
        ...row,
        a: leolabsFunding ? `~${leolabsFunding} (Series B, 2022)` : row.a,
        b: slingshotFunding ? `~${slingshotFunding} (Series B, 2023)` : row.b,
      };
    }
    return row;
  });
    ...
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      ...
      <div className="card-terminal mb-8">
        ...
        <div className="px-2 sm:px-4 pb-3">
          <CompareFiguresFootnote figures={[leolabs, slingshot]} />
        </div>
      </div>
