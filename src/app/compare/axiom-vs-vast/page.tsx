...
const COMPARISON_DATA = [
  ...
  { metric: 'First Module', a: 'Payload Power Thermal Module (PPTM) — attach to the ISS first, at NASA\'s request; the Hab One habitat follows', b: 'Haven-1 — single-module station launched independently' },
  { metric: 'Target Launch (First Module)', a: 'Early 2027 (PPTM to ISS)', b: 'NET Q1 2027 (Haven-1 on Falcon 9; slipped from 2026 during integration)' },
  { metric: 'Full Station Completion', a: 'Two-module free-flyer early 2028 (PPTM undocks and joins Hab One); more modules after', b: 'Haven-2 first module targeted 2028; multi-module by the early 2030s' },
...
export default async function Page() {
  const figures = await getCompareFigures(['axiom-space', 'vast']);
  const axiom = figures['axiom-space'];
  const vast = figures['vast'];
  const axiomFunding = formatFundingTotal(axiom?.totalFundingUSD);
  const vastFunding = formatFundingTotal(vast?.totalFundingUSD);
  const axiomValuation = formatValuation(axiom?.valuationUSD);
  const vastValuation = formatValuation(vast?.valuationUSD);

  const comparisonData = COMPARISON_DATA.map((row) => {
    if (row.metric === 'Total Funding') {
      return {
        ...row,
        a: axiomFunding ? `~${axiomFunding} (Series C, 2023)` : row.a,
        b: vastFunding ? `~${vastFunding} (including $100M Launcher acquisition)` : row.b,
      };
    }
    if (row.metric === 'Valuation') {
      return {
        ...row,
        a: axiomValuation ? `Private (est. ${axiomValuation})` : row.a,
        b: vastValuation ? `Private (est. ${vastValuation}, per last round)` : row.b,
      };
    }
    return row;
  });
    ...
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      ...
      <div className="card-terminal mb-8">
          ...
          <table className="w-full text-xs sm:text-sm">
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                ...
                <th className="py-2 sm:py-3 px-2 sm:px-4 text-center text-[11px] sm:text-xs font-bold" style={{ color: 'var(--text-primary)' }}>Vast</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div className="px-2 sm:px-4 pb-3">
          <CompareFiguresFootnote figures={[axiom, vast]} />
        </div>
      </div>
      ...
      <div className="mt-8 p-4 rounded-lg bg-white/[0.03] border border-white/[0.06]">
        ...
        <ul className="space-y-2">
          ...
          <li><Link href="/blog/sierra-space-vast-billion-dollar-raises-2026" className="text-sm text-indigo-400 hover:text-indigo-300">Sierra Space and Vast Raise Over $1 Billion: What the Funding Boom Means</Link></li>
          <li><Link href="/guide/space-economy-investment" className="text-sm text-indigo-400 hover:text-indigo-300">Investing in the Space Economy: Complete 2026 Guide</Link></li>
        </ul>
      </div>
