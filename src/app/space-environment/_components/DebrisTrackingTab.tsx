...
export default function DebrisTrackingTab() {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div className="card-elevated p-4 text-center">
          <div className="text-2xl font-bold font-display text-white">{totalTracked.toLocaleString()}</div>
          <div className="text-slate-400 text-xs uppercase tracking-widest font-medium">Tracked Objects</div>
        </div>
        <div className="card-elevated p-4 text-center">
          <div className={`text-2xl font-bold font-display ${getKesslerColor(stats?.kesslerRiskIndex || 0)}`}>
            {(stats?.kesslerRiskIndex || 0).toFixed(1)}
          </div>
          <div className="text-slate-400 text-xs uppercase tracking-widest font-medium">Kessler Index</div>
          <div className={`text-xs mt-0.5 ${getKesslerColor(stats?.kesslerRiskIndex || 0)}`}>
            {getKesslerLabel(stats?.kesslerRiskIndex || 0)}
          </div>
        </div>
        <div className="card-elevated p-4 text-center">
          <div className="text-2xl font-bold font-display text-white">{(stats?.conjunctionsPerDay || 0).toFixed(1)}</div>
          <div className="text-slate-400 text-xs uppercase tracking-widest font-medium">Conj./Day</div>
        </div>
        <div className="card-elevated p-4 text-center">
          <div className={`text-2xl font-bold font-display ${criticalCount > 0 ? 'text-red-400' : 'text-green-400'}`}>{criticalCount}</div>
          <div className="text-slate-400 text-xs uppercase tracking-widest font-medium">Critical Events</div>
        </div>
        <div className="card-elevated p-4 text-center">
          <div className="text-2xl font-bold font-display text-white">{conjunctions.length}</div>
          <div className="text-slate-400 text-xs uppercase tracking-widest font-medium">Active Conj.</div>
        </div>
        <div className="card-elevated p-4 text-center">
        ...
        <div className="space-y-6">
          ...
          {sortedConjunctions.filter((c) => c.riskLevel === 'critical' || c.riskLevel === 'high').length > 0 && (
            <div>
              ...
              <div className="space-y-4">
                {sortedConjunctions.filter((c) => c.riskLevel === 'critical' || c.riskLevel === 'high').slice(0, 4).map((event) => (
        ...
        <div>
          <div className="card p-4 mb-6">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="ml-auto">
                <ExportButton data={sortedConjunctions} filename="conjunction-events" columns={DEBRIS_CONJUNCTION_EXPORT_COLUMNS} label="Export" />
              </div>
            </div>
          </div>
          ...
          {sortedConjunctions.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-slate-400">{riskFilter ? `No ${riskFilter} risk conjunction events at this time.` : 'No active conjunction events.'}</p>
            </div>
          ) : (
            <div className="space-y-4">
        ...
        <div>
          <div className="card p-4 mb-6">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="ml-auto">
                <ExportButton data={filteredObjects} filename="debris-objects" columns={DEBRIS_EXPORT_COLUMNS} label="Export" />
              </div>
            </div>
          </div>
          ...
          {filteredObjects.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-slate-400">{objectTypeFilter ? `No ${objectTypeFilter.replace('_', ' ')} objects in the database.` : 'No tracked objects available.'}</p>
            </div>
          ) : (
