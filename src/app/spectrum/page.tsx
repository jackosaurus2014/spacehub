  ...
  if (loading) {
    return (
      <div className="min-h-screen">
        ...
        <RelatedModules modules={PAGE_RELATIONS['spectrum']} />
        </div>
      </div>
    );
  }
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {band.keyOperators.map((op) => (
                        <span
                          key={op}
                          className="text-[11px] px-2 py-0.5 rounded bg-white/5 text-slate-900/80 border border-white/5"
                        >
                          {op}
                        </span>
                      ))}
...
export default function SpectrumPage() {
  return (
    ...
    <div className="container mx-auto px-4 mt-10 mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { title: 'Satellite Frequency Bands Explained: L, S, C, X, Ku, Ka', slug: 'satellite-frequency-bands-explained-l-s-c-x-ku-ka' },
          { title: 'FCC Satellite Licensing: Spectrum & Orbital Slots', slug: 'fcc-satellite-licensing-spectrum-orbital-slots' },
          { title: 'WRC-27 Preview: Spectrum Battles for Space Communications', slug: 'wrc-27-preview-spectrum-battles-space-communications' },
        ].map((article) => (
          <Link
            key={article.slug}
