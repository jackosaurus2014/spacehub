...
interface IndustryTrendsClientProps {
  /** Latest SATCAT snapshot from the server wrapper, or null when unavailable. */
...
interface Trend {
  ...
  timeframe: string;
  /**
   * Vintage of the trend's figures — the year its metric labels / evidence
   * cite (e.g. "2025", "FY2026"). Omitted when the figures cite no year;
   * the card then shows the source line without a date. Never a guess.
   */
  figuresAsOf?: string;
  description: string;
  evidence: string[];
...
/** The trend's stated figure vintage, or null when its figures cite no year. */
function figureVintage(trend: Trend): string | null {
  return trend.figuresAsOf ?? null;
}

const TREND_SOURCES = 'Euroconsult, Bryce Tech, Space Capital, public filings · SpaceNexus editorial analysis';

/**
 * Swap in the one figure on this page the site tracks itself: the
 * sustainability trend's "tracked objects" count, replaced by the live
 * SATCAT total when the snapshot is available. Every other metric is a
 * dated reference figure and is left untouched.
 */
function withLiveMetrics(trend: Trend, satellites: SatelliteTotals | null): Trend {
  if (!satellites || trend.category !== 'Sustainability') return trend;
  return {
    ...trend,
    metrics: trend.metrics.map((m) =>
      m.label === 'Tracked debris objects'
        ? { label: 'Tracked objects in orbit (SATCAT, live)', value: satellites.totalTracked.toLocaleString('en-US') }
        : m
    ),
  };
}
...
export default function IndustryTrendsClient({ satellites }: IndustryTrendsClientProps) {
  ...
  const filteredTrends = TRENDS.map((t) => withLiveMetrics(t, satellites)).filter((trend) => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-4">
          {filteredTrends.map((trend, idx) => {
            ...
            const vintage = figureVintage(trend);
            const hasLiveMetric = !!satellites && trend.category === 'Sustainability';
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          ...
                          {vintage && (
                            <span className="text-[11px] text-slate-600 font-mono">figures as of {vintage}</span>
                          )}
                        </div>
                      ...
                      <div className="grid md:grid-cols-3 gap-5">
                        ...
                        <div>
                          ...
                          <div className="mt-3 space-y-1">
                            {vintage ? (
                              <DataAsOf date={vintage} note={`Source: ${TREND_SOURCES}`} className="!text-[10px] leading-snug" />
                            ) : (
                              <Provenance source={TREND_SOURCES} />
                            )}
                            {hasLiveMetric && satellites && (
                              <Provenance source="Space-Track SATCAT snapshot (tracked objects)" asOf={satellites.asOf} dateOnly />
                            )}
                          </div>
                        </div>
                      </div>
        ...
        {filteredTrends.length === 0 && (
          <EmptyState
            reason="Trends are refreshed as new industry data lands. Your current category and impact filters exclude every trend on file; clearing them restores results immediately."
