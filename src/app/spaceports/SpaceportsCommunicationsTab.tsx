          ...
          <div>
            <div className="card-elevated p-6 border border-space-700 mb-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="bg-space-800/50 rounded-lg p-3"><div className="text-slate-500 text-xs uppercase tracking-widest mb-1">Complexes</div><div className="text-cyan-400 text-lg font-bold">3</div></div>
                <div className="bg-space-800/50 rounded-lg p-3"><div className="text-slate-500 text-xs uppercase tracking-widest mb-1">Total Antennas</div><div className="text-cyan-400 text-lg font-bold">13</div></div>
                <div className="bg-space-800/50 rounded-lg p-3"><div className="text-slate-500 text-xs uppercase tracking-widest mb-1">70m Dishes</div><div className="text-cyan-400 text-lg font-bold">3</div></div>
                <div className="bg-space-800/50 rounded-lg p-3"><div className="text-slate-500 text-xs uppercase tracking-widest mb-1">Missions Supported</div><div className="text-cyan-400 text-lg font-bold">40+</div></div>
              </div>
            </div>
                  ...
                  <tbody>
                    {[
                      ...
                      <tr key={idx} className={`border-b border-space-800 ${idx % 2 === 0 ? 'bg-space-900/50' : ''}`}>
                        <td className="py-2 px-3 text-white font-medium">{row.type}</td><td className="py-2 px-3 text-cyan-400 font-bold">{row.diameter}</td><td className="py-2 px-3 text-slate-300">{row.bands}</td><td className="py-2 px-3 text-slate-400">{row.use}</td><td className="py-2 px-3 text-white">{row.count}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <tbody>
                    {estrackStations.map((station, idx) => (
                      <tr key={station.name} className={`border-b border-space-800 ${idx % 2 === 0 ? 'bg-space-900/50' : ''}`}>
                        <td className="py-2 px-3 text-white font-medium">{station.name}</td><td className="py-2 px-3 text-slate-300">{station.location}</td><td className="py-2 px-3 text-cyan-400 font-bold">{station.diameter}</td><td className="py-2 px-3 text-slate-300">{station.bands}</td><td className="py-2 px-3 text-slate-400">{station.role}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div>
            <div className="flex flex-wrap items-center gap-3 mb-6">
              ...
              {['', 'operational', 'deploying', 'development'].map((status) => (
                <button key={status} onClick={() => setRelayFilter(status)} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${relayFilter === status ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/50' : 'bg-space-800 text-slate-400 border border-space-700 hover:border-space-600 hover:text-white'}`}>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {[
                      ...
                      <tr key={idx} className={`border-b border-space-800 ${idx % 2 === 0 ? 'bg-space-900/50' : ''}`}>
                        <td className="py-2 px-3 text-white font-medium">{row.name}</td><td className="py-2 px-3 text-slate-300">{row.orbit}</td><td className="py-2 px-3 text-cyan-400">{row.link}</td><td className="py-2 px-3 text-amber-400 font-medium">{row.rate}</td><td className="py-2 px-3 text-slate-300">{row.coverage}</td>
                        <td className="py-2 px-3"><span className={`text-xs font-bold px-2 py-0.5 rounded ${row.status === 'Operational' ? 'text-green-400 bg-green-500/10' : row.status === 'Deploying' ? 'text-cyan-400 bg-cyan-500/10' : 'text-amber-400 bg-amber-500/10'}`}>{row.status}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {[
                      ...
                      <tr key={idx} className={`border-b border-space-800 ${idx % 2 === 0 ? 'bg-space-900/50' : ''}`}><td className="py-2 px-3 text-white font-medium">{row.param}</td><td className="py-2 px-3 text-cyan-400">{row.rf}</td><td className="py-2 px-3 text-amber-400">{row.optical}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
            ...
            <div className="card-elevated p-6 border border-cyan-500/20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-space-800/50 rounded-xl p-4"><div className="text-cyan-400 text-2xl font-bold mb-1">10-100x</div><div className="text-slate-400 text-sm">Faster than RF at comparable distance</div><p className="text-slate-500 text-xs mt-2">At 33 million km, DSOC achieved 267 Mbps vs. ~2-25 Mbps for X-band RF systems.</p></div>
                <div className="bg-space-800/50 rounded-xl p-4"><div className="text-cyan-400 text-2xl font-bold mb-1">2.6 AU</div><div className="text-slate-400 text-sm">Maximum demonstrated range</div><p className="text-slate-500 text-xs mt-2">390 million km -- nearly 2x the Earth-Sun distance, well beyond Mars distance.</p></div>
                <div className="bg-space-800/50 rounded-xl p-4"><div className="text-cyan-400 text-2xl font-bold mb-1">1.58 Tb</div><div className="text-slate-400 text-sm">Total data transmitted</div><p className="text-slate-500 text-xs mt-2">Cumulative data return during the technology demonstration phase in 2024.</p></div>
              </div>
            </div>
          </div>
        )}
        ...
        {commsSubTab === 'lunar' && (
          <div>
            ...
            <div className="card-elevated p-6 border border-space-700 mb-6">
              ...
              <div className="space-y-3">{[{ phase: 'Artemis I (2022)', comms: 'Direct DSN links via Orion S-band/Ka-band', status: 'Completed', color: 'text-green-400' },{ phase: 'Artemis II (2025)', comms: 'DSN + TDRS relay for crew safety during Earth transit', status: 'Upcoming', color: 'text-cyan-400' },{ phase: 'Artemis III (2026)', comms: 'DSN direct + potential Starlink supplement for video relay', status: 'Planned', color: 'text-amber-400' },{ phase: 'Gateway Ops (2027+)', comms: 'Gateway PPE Ka-band relay + proximity S-band to surface', status: 'Development', color: 'text-amber-400' },{ phase: 'LunaNet Era (2030+)', comms: 'Multi-provider relay constellation with DTN networking', status: 'Planned', color: 'text-purple-400' }].map((item, idx) => (<div key={idx} className="flex items-start gap-4 bg-space-800/30 rounded-lg p-4"><div className="flex-shrink-0 w-2 h-2 rounded-full bg-purple-400 mt-2" /><div className="flex-1"><div className="flex items-center gap-3 mb-1"><span className="text-white font-semibold text-sm">{item.phase}</span><span className={`text-xs font-bold ${item.color}`}>{item.status}</span></div><p className="text-slate-400 text-sm">{item.comms}</p></div></div>))}</div>
            </div>
