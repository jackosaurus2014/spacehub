        ...
        {commsSubTab === 'lunar' && (
          <div>
            ...
            <div className="card-elevated p-6 border border-space-700 mb-6">
              ...
              <div className="space-y-3">{[{ phase: 'Artemis I (2022)', comms: 'Direct DSN links via Orion S-band/Ka-band', status: 'Completed', color: 'text-green-400' },{ phase: 'Artemis II (2026)', comms: 'DSN + TDRS relay for crew safety during Earth transit; SpaceX Starlink selected for 4K video downlink from Orion', status: 'Completed', color: 'text-green-400' },{ phase: 'Artemis III (NET late 2027)', comms: 'DSN direct + Starlink supplement for video relay during Earth-orbit HLS demonstration', status: 'Upcoming', color: 'text-slate-300' },{ phase: 'Gateway Ops (2028+)', comms: 'Gateway PPE Ka-band relay + proximity S-band to surface', status: 'Development', color: 'text-amber-400' },{ phase: 'LunaNet Era (2030+)', comms: 'Multi-provider relay constellation with DTN networking', status: 'Planned', color: 'text-purple-400' }].map((item, idx) => (<div key={idx} className="flex items-start gap-4 bg-space-800/30 rounded-lg p-4"><div className="flex-shrink-0 w-2 h-2 rounded-full bg-purple-400 mt-2" /><div className="flex-1"><div className="flex items-center gap-3 mb-1"><span className="text-white font-semibold text-sm">{item.phase}</span><span className={`text-xs font-bold ${item.color}`}>{item.status}</span></div><p className="text-slate-400 text-sm">{item.comms}</p></div></div>))}</div>
            </div>
