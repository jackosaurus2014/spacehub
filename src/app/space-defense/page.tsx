...
// Lazy-load the Procurement tab (~270 lines, only visible when tab selected)
const ProcurementTab = dynamic(() => import('./ProcurementTab'), {
  ssr: false,
  loading: () => <div className="animate-pulse space-y-4"><div className="h-48 bg-white/[0.06] rounded-lg" /><div className="grid grid-cols-2 gap-4">{[1,2,3,4].map(i => <div key={i} className="h-32 bg-white/[0.06] rounded-lg" />)}</div></div>,
});
...
const FALLBACK_SPACE_FORCES: SpaceForce[] = [
  {
    ...
    description: 'The United States Space Force (USSF) is the space service branch of the U.S. Armed Forces, one of the eight uniformed services, and the world\'s first and currently only independent space force. Established on December 20, 2019, the USSF organizes, trains, and equips space forces to protect U.S. and allied interests in space and to provide space capabilities to the joint force. The FY2026 budget request of $29.4 billion reflects growing emphasis on space as a warfighting domain.',
    fieldCommands: [
...
const FALLBACK_DEFENSE_PROGRAMS: DefenseProgram[] = [
  ...
  },
  {
    ...
    nextMilestone: 'GEO-1 launch \u2014 still pending as of Aug 2026',
  ...
  },
  {
    ...
    status: 'operational',
    category: 'launch',
    budget: '$5.6B (Phase 2 awards, FY2026-2029)',
    description: 'NSSL ensures assured access to space for the most critical national security payloads. Phase 2 split launches 60/40 between ULA and SpaceX. Phase 3 (Lane 1 for smaller payloads) added Rocket Lab, Firefly, and Stoke Space alongside SpaceX. Vulcan Centaur completed NSSL certification and is now flying operational national security missions; Blue Origin\'s New Glenn is entering NSSL certification next. NSSL supports launches from CCSFS, VSFB, and Wallops.',
    nextMilestone: 'Blue Origin New Glenn NSSL certification \u2014 in progress',
  },
  {
    id: 'gssap',
  ...
  },
  {
    ...
    nextMilestone: 'Skynet 6A launch \u2014 date TBC, still pending as of Aug 2026',
  },
  {
    id: 'cso',
...
const FALLBACK_RECENT_CONTRACTS: ContractAward[] = [
  ...
  },
  {
    ...
    agency: 'USSF / Space Systems Command',
    category: 'Launch Services',
    description: 'ULA was awarded ~40% of NSSL Phase 2 launch service agreements using Atlas V and Vulcan Centaur. Vulcan Centaur has completed NSSL certification and is now flying operational national security missions, taking over from Atlas V, which is being retired after remaining orders.',
    period: 'FY2022-2027',
  },
  {
