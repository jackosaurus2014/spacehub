...
// Client island for /satellites — the live canvas map, search/filter sidebar,
// featured-satellite cards and everything below them. The crawlable first
// screen (h1, daily-SATCAT catalog stats, notable objects) is server-rendered
// by page.tsx. This island fetches its own TLE set: the CelesTrak cache is
// in-memory inside /api/satellites/tle (rate-limited upstream), so the client
// fetch stays its single consumer — no server seed, hence no skipFirstFetch
// guard needed.
...
export default function SatellitesClient() {
    ...
    <>
      <div className="min-h-screen">
        <div className="container mx-auto px-4">
          {/* Live telemetry strip — counts for the propagated set currently on
              the map. The catalog-level stats bar (full SATCAT totals) is
              server-rendered by page.tsx; this strip is only ever about the
              live TLE set, so pre-JS it renders nothing rather than zeros. */}
          {!loading && satellites.length > 0 && (
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mb-1 text-xs text-slate-400">
              <span className="text-white font-semibold">{stats.total.toLocaleString()} on the map</span>
              <span><span className="text-blue-400 font-semibold">{stats.leo}</span> LEO</span>
              <span><span className="text-yellow-400 font-semibold">{stats.meo}</span> MEO</span>
              <span><span className="text-fuchsia-400 font-semibold">{stats.geo}</span> GEO</span>
              <span><span className="text-green-400 font-semibold">{stats.heo}</span> HEO</span>
              {iss && (
                <span className="font-mono">
                  ISS: {iss.position.lat.toFixed(1)}, {iss.position.lng.toFixed(1)} · {iss.position.altitude.toFixed(0)} km
                </span>
              )}
            </div>
          )}
