...
export default function Error({ error, reset }: { error: Error; reset: () => void }) {
  useEffect(() => {
    clientLogger.error('Page error', { page: 'reports/space-economy-2026', message: error.message });
  }, [error]);
    ...
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center space-y-4">
        ...
        <div className="flex gap-3 justify-center">
          ...
          <Link href="/reports" className="px-4 py-2 bg-white/[0.08] hover:bg-white/[0.1] text-white text-sm rounded-lg transition-colors">
            Go Back
          </Link>
        </div>
      </div>
