...
// Lazy-load companies/technologies/resources grid components (below-the-fold tabs)
const CompaniesGrid = dynamic(() => import('./MiningInfoTabs').then(m => ({ default: m.CompaniesGrid })), {
  ssr: false,
  loading: () => <div className="animate-pulse grid grid-cols-2 gap-4">{[1,2,3,4].map(i => <div key={i} className="h-48 bg-white/[0.06] rounded-lg" />)}</div>,
});
const TechnologiesGrid = dynamic(() => import('./MiningInfoTabs').then(m => ({ default: m.TechnologiesGrid })), {
  ssr: false,
  loading: () => <div className="animate-pulse grid grid-cols-2 gap-4">{[1,2,3,4].map(i => <div key={i} className="h-48 bg-white/[0.06] rounded-lg" />)}</div>,
});
const ResourcesGrid = dynamic(() => import('./MiningInfoTabs').then(m => ({ default: m.ResourcesGrid })), {
  ssr: false,
  loading: () => <div className="animate-pulse grid grid-cols-2 gap-4">{[1,2,3,4].map(i => <div key={i} className="h-48 bg-white/[0.06] rounded-lg" />)}</div>,
});
import {
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'companies' ? (
        ...
        <div>
          ...
          <TechnologiesGrid technologies={filteredTechnologies} />
        </div>
      ) : activeTab === 'resources' ? (
        <div>
          <div className="flex items-center justify-between mb-4">
