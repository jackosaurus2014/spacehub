    ...
    <div className="flex flex-col items-center">
      <div className="relative w-52 h-52">
        <svg viewBox="0 0 200 200" className="w-full h-full -rotate-[135deg]">
          <circle cx="100" cy="100" r="80" fill="none" stroke="rgba(148, 163, 184, 0.15)" strokeWidth="16" strokeDasharray={`${circumference * 0.75} ${circumference * 0.25}`} strokeLinecap="round" />
          <circle
            cx="100"
            cy="100"
            r="80"
      ...
      <button onClick={() => setExpanded(!expanded)} className="w-full text-left flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="w-32 h-2 bg-white/[0.08] rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-700" style={{ width: `${barWidth}%`, backgroundColor: config.color }} />
          </div>
          <span className="text-lg font-bold text-white/90 w-10 text-right">{score}</span>
          <svg className={`w-5 h-5 text-slate-400 transition-transform duration-200 ${expanded ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {factors.map(f => (
            <div key={f.factorId} className="bg-black/50 rounded-lg p-3">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-sm font-bold" style={{ color: f.score >= 80 ? '#ef4444' : f.score >= 60 ? '#f97316' : f.score >= 40 ? '#f59e0b' : '#10b981' }}>{f.score}</span>
              </div>
...
function TimelineChart({ applicableFactors }: { applicableFactors: string[] }) {
  const items = applicableFactors
    ...
    .map(id => {
      ...
      return { id, name: factor?.name || id, min: timeline.min, max: timeline.max, unit: timeline.unit };
    })
            ...
            <div key={item.id} className="flex items-center gap-4">
              ...
              <div className="flex-1 relative h-6">
                ...
                <div
                  ...
                  style={{ left: `${(item.min / maxVal) * 100}%`, width: `${Math.max(widthPct, 2)}%`, backgroundColor: `hsl(${200 - i * 15}, 70%, 55%)`, opacity: 0.8 }}
                />
                <div className="absolute top-0 h-full w-0.5 bg-slate-400" style={{ left: `${leftPct}%` }} />
              </div>
              ...
              {factors.map(f => (
                <div key={f.id}>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-medium text-white/90">{f.name}</span>
                      <span className="text-xs px-2 py-0.5 rounded-full bg-white/[0.06] text-slate-400">Weight: {f.weight}/10</span>
                    </div>
                    <svg className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${expandedId === f.id ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                    </svg>
                  </button>
                  {expandedId === f.id && (
                    <div className="px-5 pb-4 space-y-2">
                      ...
                      {LICENSE_TIMELINES[f.id] && (
                        <p className="text-xs text-slate-500">Estimated timeline: {LICENSE_TIMELINES[f.id].min}-{LICENSE_TIMELINES[f.id].max} {LICENSE_TIMELINES[f.id].unit}</p>
                      )}
                    </div>
                  )}
                </div>
...
export function RiskAssessmentTab() {
  ...
  const toggleActivity = useCallback((actId: string) => {
    setSelectedActivities(prev => (prev.includes(actId) ? prev.filter(a => a !== actId) : [...prev, actId]));
  }, []);
  ...
  const runAssessment = useCallback(() => {
    ...
    setIsAssessing(true);
    setTimeout(() => {
      const profile: CompanyRiskProfile = { sector: selectedSector, activitiesFlags: selectedActivities };
  ...
  const applicableFactorIds = assessment ? assessment.categoryScores.flatMap(cs => cs.factors.map(f => f.factorId)) : [];

  return (
    <div>
      <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4 mb-6">
        <h4 className="font-semibold text-amber-400 mb-2">Regulatory Risk Assessment</h4>
        <p className="text-sm text-slate-400">Evaluate regulatory complexity and licensing requirements for space industry operations. Identify risks, estimate timelines, and get actionable compliance recommendations.</p>
      </div>
      ...
      <div className="flex gap-2 mb-8">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeSection === 'form' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-white/[0.04] text-slate-400 border border-white/[0.06] hover:text-white/90'}`}
        >
          Profile Builder
        </button>
        <button
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeSection === 'reference' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-white/[0.04] text-slate-400 border border-white/[0.06] hover:text-white/90'}`}
        >
      ...
      {activeSection === 'form' && (
        <div className="card p-6 md:p-8">
          ...
          <div className="mb-8">
            <label className="block text-sm font-medium text-white/70 mb-3">Primary Sector <span className="text-red-400">*</span></label>
              ...
              {SECTORS.map(s => (
                <button
                  ...
                  className={`px-4 py-3 rounded-xl text-left text-sm font-medium transition-all border ${
                    selectedSector === s.id ? 'bg-white/10 text-white/90 border-white/15 shadow-lg shadow-black/5' : 'bg-white/[0.04] text-white/70 border-white/[0.06] hover:bg-white/[0.08] hover:text-slate-100'
                  }`}
                >
                  {s.label}
                </button>
          ...
          <div className="mb-8">
            <label className="block text-sm font-medium text-white/70 mb-1">Company Activities</label>
            <p className="text-xs text-slate-500 mb-3">Select all activities that apply. These refine the risk assessment beyond the base sector profile.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {ACTIVITY_FLAGS.map(a => (
                <label
                  key={a.id}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-all border ${
                    selectedActivities.includes(a.id) ? 'bg-amber-500/10 border-amber-500/30 text-amber-200' : 'bg-white/[0.04] border-white/[0.06] text-slate-400 hover:text-white/90 hover:bg-white/[0.06]'
                  }`}
                >
                  <input
                    type="checkbox"
          ...
          <div className="flex items-center gap-4">
            ...
            {assessment && (
              <button onClick={resetForm} className="px-4 py-3 rounded-xl text-sm text-slate-400 hover:text-white/90 transition-colors">Reset</button>
            )}
          </div>
      ...
      {activeSection === 'results' && assessment && (
        <div ref={resultsRef} className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="card p-6 flex flex-col justify-center space-y-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-100">{assessment.categoryScores.reduce((sum, c) => sum + c.factors.length, 0)}</p>
              </div>
            </div>

            <div className="card p-6">
              <h3 className="text-sm font-semibold text-white/70 uppercase tracking-wider mb-4">Required Licenses & Authorizations</h3>
          ...
          <div>
            ...
            <div className="space-y-3">
              {assessment.categoryScores.map(cs => (
                <CategoryScoreCard key={cs.category} category={cs.category} score={cs.score} factors={cs.factors} />
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <button onClick={() => setActiveSection('form')} className="px-5 py-2.5 rounded-xl text-sm font-medium bg-white/[0.04] text-white/70 border border-white/[0.06] hover:bg-white/[0.08] hover:text-slate-100 transition-all">
              Modify Profile
            </button>
            <button onClick={resetForm} className="px-5 py-2.5 rounded-xl text-sm font-medium bg-white/[0.04] text-white/70 border border-white/[0.06] hover:bg-white/[0.08] hover:text-slate-100 transition-all">
              New Assessment
            </button>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="card p-6">
            ...
            <p className="text-sm text-slate-400 mb-6">
              Complete reference of all {RISK_FACTORS.length} regulatory risk factors assessed by the engine. Each factor has a weight (1-10) reflecting its regulatory impact severity.
            </p>
