...
export default function LaunchListPage() {
  ...
  // Build Event structured data for upcoming launches (first 5)
  const schemaEvents = [...data.live, ...allUpcoming].slice(0, 5);
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950">
      {schemaEvents.map((event) => (
        <EventSchema
          key={`schema-${event.id}`}
          name={event.name}
          description={event.description || `${event.name} - space launch event`}
          startDate={event.launchDate || new Date().toISOString()}
          location={event.location || undefined}
          organizer={event.agency || 'Unknown'}
          url={event.infoUrl || `https://spacenexus.us/launch/${event.id}`}
