    ...
    <div className="min-h-screen py-8">
      <div className="container mx-auto px-4">
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4">
          ...
          <div className="h-12 w-40 bg-slate-800/50 rounded-xl animate-pulse" />
        </div>

        {/* Category tabs skeleton */}
        <div className="flex gap-2 mb-8 overflow-x-auto">
          {Array.from({ length: 7 }).map((_, i) => (
            <div key={i} className="h-9 w-24 bg-slate-800/40 rounded-lg animate-pulse flex-shrink-0" />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {Array.from({ length: 9 }).map((_, i) => (
            <div key={i} className="h-52 bg-slate-800/40 rounded-xl animate-pulse p-5 flex flex-col justify-between">
