...
export default function BOMRiskAnalysisTab() {
  // Self-contained state for BOM Risk tab
  ...
  const bomRiskStats = getBOMRiskStats();
  const bomCategories = getAllBOMCategories();
  const bomSystems = getAllUsedInSystems();
                  ...
                  <div className="flex flex-wrap gap-2">
                    {item.supplierCompanyIds.map((slug) => (
                      <Link
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 text-slate-200 text-xs font-medium border border-white/10 hover:bg-slate-100/20 hover:border-white/15 transition-colors"
                      >
