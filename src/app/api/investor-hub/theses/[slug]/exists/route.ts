...
// unknown investor thesis ids get a real HTTP 404 instead of a soft 200.
...
export async function GET(_request: NextRequest, props: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const row = await prisma.investorThesis.findFirst({
    ...
    logger.error('Investor thesis existence check failed', {
