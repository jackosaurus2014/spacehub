...
const stats = { fundingRounds: 0, products: 0, personnel: 0, events: 0, facilities: 0, contracts: 0, revenueEstimates: 0, scores: 0 };

async function enrichCompany(data: EnrichmentData): Promise<boolean> {
  ...
  if (!profile) {
    logger.warn('Company not found for enrichment', { slug: data.slug });
    return false;
  }
  ...
  if (data.facilities) {
    for (const f of data.facilities) {
      try {
        await db.facilityLocation.create({
          data: {
            companyId: profile.id,
            name: f.name,
            type: f.type,
  ...
  if (data.contracts) {
    for (const c of data.contracts) {
      try {
        await db.governmentContractAward.create({
          data: {
            companyId: profile.id,
  ...
  if (data.revenueEstimates) {
    for (const r of data.revenueEstimates) {
      try {
        await db.revenueEstimate.upsert({
...
const TIER_2_ENRICHMENTS: EnrichmentData[] = [
  ...
  },
  {
    ...
    events: [
      ...
      { date: '2023-06-01', type: 'funding', title: 'Raises EUR40M Series B for launch development', importance: 6 },
    ],
    facilities: [
...
const TIER_3_ENRICHMENTS: EnrichmentData[] = [
  ...
  },
  {
    ...
    keyPersonnel: [
      { name: 'Raul Verdu', title: 'Co-Founder & CTO', role: 'founder' },
    ],
    events: [
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Reset stats
    stats.fundingRounds = 0;
    stats.products = 0;
    stats.personnel = 0;
    stats.events = 0;
    stats.facilities = 0;
    stats.contracts = 0;
    stats.revenueEstimates = 0;
    stats.scores = 0;

    let enrichments: EnrichmentData[] = [];

    if (tier === '2' || tier === 'all') {
      enrichments = enrichments.concat(TIER_2_ENRICHMENTS);
    }
    if (tier === '3' || tier === 'all') {
      enrichments = enrichments.concat(TIER_3_ENRICHMENTS);
    }

    if (enrichments.length === 0) {
      return createSuccessResponse({
        message: `No enrichments for tier=${tier}. Valid values: 2, 3, all`,
        enriched: 0,
        failed: 0,
        stats,
      });
    ...
    logger.info('Starting company profile enrichment', {
      tier,
      totalCompanies: enrichments.length,
    });
    ...
    for (const data of enrichments) {
      try {
        const success = await enrichCompany(data);
        if (success) {
          enriched++;
        } else {
          failed++;
        ...
        failed++;
        logger.error('Failed to enrich company', {
          slug: data.slug,
    ...
    logger.info('Company profile enrichment complete', {
      enriched,
      failed,
      stats,
    });
    ...
    return internalError('Company profile enrichment failed');
  }
}
