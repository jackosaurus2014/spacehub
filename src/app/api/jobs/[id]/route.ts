...
const patchSchema = z.object({
  ...
  applyUrl: z.string().trim().url().max(500).optional(),
...
async function ownedPosting(id: string, userId: string) {
  const row = await prisma.spaceJobPosting.findUnique({
    where: { id },
    select: { id: true, postedByUserId: true, source: true, paidAt: true, expiresAt: true, isActive: true, salaryMin: true, salaryMax: true },
  });
  if (!row || row.source !== 'direct' || row.postedByUserId !== userId) return null;
  return row;
}
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const b = parsed.data;
  try {
    const row = await ownedPosting(id, session.user.id);
