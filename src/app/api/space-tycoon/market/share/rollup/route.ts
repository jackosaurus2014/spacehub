 ...
 * layer for market-share.ts's heavier reports). Same cron-secret gating
 * pattern as /api/space-tycoon/leagues/process-week. Idempotent (upsert) —
 * safe to re-run or to run against a specific `day` for backfill.
 *
 * Body: { secret: string, day?: string (ISO date) }
 *
