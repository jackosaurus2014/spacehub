...
import { unsubscribeCompanyWatch } from '@/lib/company-brief';
...
async function handle(request: NextRequest) {
  ...
  const r = await unsubscribeCompanyWatch(token, all);
  if (!r.ok) return page('Nothing to stop', 'That brief was already removed.');
  return page('Briefs stopped', all ? `Removed ${r.count} company brief${r.count === 1 ? '' : 's'} for this address.` : `That brief is removed. <a href="?token=${token}&all=1">Stop all company briefs for this address</a> if you want none at all.`);
}
