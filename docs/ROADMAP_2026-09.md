...
| 11 | **Iterate the two pages that carry the site** — launch-cost and launch-schedule guides: refresh, deepen, add the slip strip and rocket links. — **DONE 2026-09-06 (75591813): both guides read the tracker at request time — cadence stat cards, providers vs same-date-last-year, flown-per-month + next 30 days + Cape block, slip ledger strip, live vehicle scorecard, rocket links, next-launch alert form, LAST_EDITED instead of now(). Verified live: 219 YTD vs 216, 12 providers, 22 launches next 30 days.** | 54k impressions; neither fleet proposed it until the skeptic caught the omission. | Sonnet's skeptic, right. |
| 12 | **Cross-link rail on every launch surface**: predict this launch → set alert → track after liftoff → rocket page → viewing guide. Wire `RelatedModules` into compare and guide templates (150 relations defined, 13 call sites). — **DONE 2026-09-06: LaunchCrossLinks already existed on launch/site/rocket pages; it gained the missing "Get launch alerts" step (anchors to the host page's LaunchWatchForm), the 8-item/4-column cap so the debrief is not pushed off, and now sits on both carrier guides, the scorecard guide and Mission Control's next-launch block.** | Only the game averages 5+ views/user; everything else is a dead end. | Both agree. |
| 13 | **Predictions without the sign-in wall**: let anyone pick an answer; stake only on sign-in. FAQ schema + numeric titles on the top-impression compare pages. — **DONE 2026-09-06 (26d77337 + follow-up): anonymous picks were already stored in localStorage by PredictionPicker (8/28). Second half shipped: FAQPage schema + numeric titles on the ten launch/telecom compare pages (was 1 of 44); the two Starship-vs-New-Glenn pages retitled for distinct intents. Lesson: page.tsx metadata overrides layout.tsx — four retitles only took effect after syncing the page.** | `predictions/page.tsx` sends every option to the game; compare CTR <1.3%. | Both agree. |
...
- **2026-09-08 (late) Trial parity with Stripe (SHIPPED, 807264a5):** one trial length (`TRIAL_DAYS`=14) across registration, the start-trial action, pricing copy and Stripe checkout, which now honours the remaining site trial as `trial_end`; expiry keeps the one-trial marker; module gate table trimmed to routes that are really gated; next-auth session endpoints get their own rate-limit budget. Live Stripe verified: prices match, FOUNDER50 inactive, no discount drift.
- **2026-09-08 (pm) Pro truth (SHIPPED):** a free-account walkthrough proved funding rounds, patents, procurement, insurance, AI insights, Space Score, marketplace, launch tracking and CSV export are open; the matrix and plan cards now list only real Pro exclusives (supply-chain map, regulatory/compliance suite, customer discovery, alerts+webhooks, API, watchlists, ad-free, support). Persona pages corrected (dead /companies link, inflated feature counts, "200+ sources"). Open: /developer/explorer React #418; session endpoint under the generic 200/min API limit.
- **2026-09-08 accuracy/value pass (SHIPPED):** /pricing matrix corrected to the enforced limits (8 cells + 2 FAQ answers were wrong: 15 articles/day, no profile cap, ungated satellite catalog, 25+10 alerts, 5,000 req/mo API, CSV+JSON only, no custom dashboards); 20 pages moved off `dateModified: new Date()` with a guard test; `/solutions/educators` added for teachers/professors/students.
- `/embed/launch-calendar` + `/widgets` listings for both data embeds — **SHIPPED 2026-09-07** (ideas #9). Checked and already working: #16 (LandingReturnsPanel on /admin/analytics), #14 (debriefs cron every 6h, cap 6, latest is yesterday's launch).
- `/guide/when-does-the-iss-retire` — **SHIPPED 2026-09-07**: the 2030 plan vs the Senate's 2032 draft, Ignition, the GAO gap warning, the Aug 9 deorbit audit, and a dated status table for every successor; the March blog post 301s here. The demand-pass list is complete.
